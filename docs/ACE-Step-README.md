...
## 🎛️ Sudo Ninja Studio — modern web frontend

This fork ships **Sudo Ninja Studio**, a polished React + TypeScript + Vite + Tailwind
frontend for the ACE-Step API. Developed by **Sudo Ninja Studio**.
...
Full docs, deployment guidance, and a Dockerfile are in
[`frontend/README.md`](frontend/README.md). A full-stack compose file is at
[`docker-compose.sudo-ninja.yml`](docker-compose.sudo-ninja.yml). CI for the
frontend is defined in [`.github/workflows/frontend.yml`](.github/workflows/frontend.yml).

The frontend is a thin client — it consumes the existing FastAPI endpoints in
[`acestep/api_server.py`](acestep/api_server.py) and never modifies backend
behaviour. See the "Endpoints consumed" table in the frontend README.
