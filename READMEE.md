...
| Screen | What's there |
|---|---|
| **Studio** | The big animated hero, mode selector (Simple / Custom / Remix / Repaint), and the full composition form — presets, model picker, live progress and results. |
| **Settings** | Wire the backend URL/key, set generation defaults, switch language (English / हिन्दी), and clear local data. |
| **History & Library** | Every generation saved locally — playable in place, downloadable, and reusable. |
| **About** | Version, credits, and honest notes on limits. |
| **Fully responsive** | Works on mobile — the sidebar collapses to a drawer. |

> 💡 Want screenshots in this README? Run the app (see **Quick start**), grab a few shots, drop them in `docs/screenshots/`, and reference them here.
